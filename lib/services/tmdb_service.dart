...
class TmdbService {
  ...
  Future<List<MovieModel>> searchPopular({int page = 1}) async {
    final url = Uri.parse('https://api.themoviedb.org/3/movie/popular?api_key=$apiKey&page=$page');
    final res = await http.get(url);
    if (res.statusCode != 200) throw Exception('TMDB error ${res.statusCode}');
    final data = jsonDecode(res.body);
    final results = (data['results'] as List).map((m) => MovieModel.fromMap(m)).toList();
    return results;
  }

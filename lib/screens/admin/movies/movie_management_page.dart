...
class _AdminMoviesPageState extends State<AdminMoviesPage> {
  ...
  Future<void> importFromTmdb() async {
    ...
    if (tmdbApiKey == null) {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('TMDB API Key not found!')));
       return;
    }
    setState(() => loading = true);
    try {
      // fetch from TMDB
      final url = Uri.parse('https://api.themoviedb.org/3/movie/$id?api_key=$tmdbApiKey&language=fr-FR');
      ...
      if (res.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('TMDB error')));
      }
    } catch (e) {
        ...
        children: [
          Row(
            children: [
              ...
              ElevatedButton(
                ...
                child: loading ? const CircularProgressIndicator(color: Colors.black) : const Text('Importer'),
              )
            ],
          ),
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: moviesRef.orderBy('createdAt', descending: true).snapshots(),
              builder: (context, snapshot) {
                if (!snapshot.hasData) return const Center(child: CircularProgressIndicator(color: Color(0xFF53FC18)));
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: d['poster'] != null ? Image.network('https://image.tmdb.org/t/p/w200${d['poster']}', width: 50, fit: BoxFit.cover) : null,
                        title: Text(d['title'] ?? '', style: const TextStyle(color: Colors.white)),
                        subtitle: Text(d['type'] ?? 'movie', style: const TextStyle(color: Colors.white54)),
          ...
          const SizedBox(height: 12),
          ElevatedButton(
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => AdminMovieSearchPage())),
            style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF53FC18), foregroundColor: Colors.black),
